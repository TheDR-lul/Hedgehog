...
// Updated to match Bybit v5 API: `result.list`
#[derive(Deserialize)]
...
impl Bybit {
    ...
    async fn call_api<T: for<'de> Deserialize<'de>>(
        ...
        if a.ret_code != 0 {
            Err(anyhow!("Bybit API error {}: {}", a.ret_code, a.ret_msg))
        } else {
            Ok(a.result)
        }
    }
...
impl Exchange for Bybit {
    ...
    async fn get_all_balances(&self) -> Result<Vec<(String, Balance)>> {
        ...
        res.list.into_iter()
            .map(|e| {
                let free = e.available.parse::<f64>()?;
                let total = e.wallet.parse::<f64>()?;
                Ok((e.coin, Balance { free, locked: total - free }))
            })
            .collect()
    }
    ...
    async fn get_funding_rate(&self, sym: &str, days: u16) -> Result<f64> {
        let res: FundingResult = self
            ...
            .await?;
        let sum = res.list.iter()
            .map(|r| r.rate.parse::<f64>().unwrap_or(0.0))
            .sum::<f64>();
    ...
    async fn place_limit_order(&self, symbol: &str, side: OrderSide, qty: f64, price: f64) -> Result<Order> {
        ...
        let r: OrderApiResult = self.call_api(
            Method::POST,
            "v5/order/create",
            None,
            Some(json!(body)),
