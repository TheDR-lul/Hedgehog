...
use super::{
    StateStorage, UserState, RunningOperations, RunningOperationInfo, OperationType, callback_data, // Убран Command
   navigation,
   wallet_info, // <<<--- ДОБАВЛЕН ИМПОРТ
   // progress, // TODO: Раскомментировать если будет использоваться
   // utils,    // TODO: Раскомментировать если будет использоваться
};
use crate::config::Config;
use crate::exchange::Exchange; // Убран Balance
use crate::storage::{Db, insert_hedge_operation}; // Убраны update_hedge_final_status, update_hedge_spot_order
...
use teloxide::types::{
   ...
   MaybeInaccessibleMessage // <<<--- ДОБАВЛЕН ИМПОРТ
};
// use teloxide::utils::command::BotCommands; // Убран
...
async fn spawn_hedge_task<E>(
   ...
   waiting_message: MaybeInaccessibleMessage, // <<<--- ИЗМЕНЕН ТИП
)
where
   E: Exchange + Clone + Send + Sync + 'static,
{
   // Используем waiting_message.id и waiting_message.chat.id если нужно
   let bot_message_id = waiting_message.id(); // Поле доступно
   let _message_chat_id = waiting_message.chat().id; // Поле доступно

   let hedger = Hedger::new((*exchange).clone(), (*cfg).clone());
   let operation_id_result = insert_hedge_operation(
       db.as_ref(),
       chat_id.0, // Используем chat_id переданный в функцию
       &params.symbol,
       &cfg.quote_currency,
       initial_sum,
   ...
   let operation_id = match operation_id_result {
       ...
       Err(e) => {
           ...
           let _ = bot.edit_message_text(chat_id, bot_message_id, error_text) // Используем bot_message_id
   ...
   let progress_callback: HedgeProgressCallback = Box::new(move |update: HedgeProgressUpdate| {
       ...
       let msg_id = bot_message_id; // Используем bot_message_id
       let chat_id_for_callback = chat_id; // Используем chat_id
       let sum = initial_sum;
       let symbol = symbol_clone.clone();
       let qc = cfg_clone.quote_currency.clone();
       let operation_id_for_callback = operation_id;
       ...
       async move {
           ...
           let status_text = if update.is_replacement { "(Ордер переставлен)" } else { "" };
            ...
            let text = format!(
                ...
                operation_id_for_callback, // 1
                progress_bar,             // 2
                sum,                      // 3
                qc,                       // 4
                symbol,                   // 5 - Символ актива
                update.current_spot_price,// 6 - Текущая цена
                update.new_limit_price,   // 7 - Цена лимитки
                status_text,              // 8 - Статус ордера (переставлен/нет)
                update.filled_qty,        // 9 - Исполнено qty
                update.target_qty,        // 10 - Цель qty
                filled_percent            // 11 - Процент
            );
           ...
           if let Err(e) = bot_for_callback.edit_message_text(chat_id_for_callback, msg_id, text)
               .reply_markup(kb)
               // .parse_mode(ParseMode::MarkdownV2) // Опционально, если нужны спецсимволы
               .await {
       ...
       match result {
           Ok((spot_qty_gross, fut_qty_net, final_spot_value_gross)) => {
                ...
                let success_text = format!(
                    ...
                    volatility_percent,
                    spot_qty_gross,
                    final_net_spot_balance,
                    fut_qty_net,
                );
                let _ = bot.edit_message_text(chat_id, bot_message_id, success_text) // Используем bot_message_id
                           .reply_markup(navigation::make_main_menu_keyboard())
                           .await;
            }
            Err(e) => {
   ...
   let info = RunningOperationInfo {
       ...
       bot_message_id: bot_message_id.0, // Сохраняем i32
...
pub async fn handle_hedge_command<E>(
   ...
   running_operations: RunningOperations, // Этот аргумент используется ниже
   cfg: Arc<Config>,
   db: Arc<Db>,
) -> anyhow::Result<()>
where
   ...
   E: Exchange + Clone + Send + Sync + 'static,
{
   if let Some(msg) = q.message {
       ...
       prompt_asset_selection(bot, chat_id, state_storage, exchange, cfg, db, Some(msg.id())).await?; // Используем поле
   } else {
   ...
   E: Exchange + Clone + Send + Sync + 'static,
{
   ...
   match wallet_info::get_formatted_balances(exchange.as_ref(), &cfg.quote_currency, false).await { // Используем wallet_info
       Ok((_, asset_data)) => {
           ...
           if !assets_found {
                // ИСПРАВЛЕНО: Добавлен аргумент для format!
            ...
            // TODO: buttons.push(vec![InlineKeyboardButton::callback("🌐 Показать все пары", callback_data::VIEW_ALL_PAIRS)]);
            buttons.push(vec![InlineKeyboardButton::callback("⬅️ Назад", callback_data::BACK_TO_MAIN)]);
            let keyboard = InlineKeyboardMarkup::new(buttons);

           if let Some(msg_id) = bot_message_id {
                if let Err(e) = bot.edit_message_text(chat_id, msg_id, &text).reply_markup(keyboard.clone()).await { // Добавил &text и clone()
   ...
   if let (Some(data), Some(msg)) = (q.data, q.message) {
       ...
       if let Some(symbol) = data.strip_prefix(callback_data::PREFIX_HEDGE_ASSET) {
           ...
           if is_correct_state {
               let text = format!("Введите сумму {} для хеджирования {}:", cfg.quote_currency, symbol);
               let kb = make_dialog_keyboard();
               bot.edit_message_text(chat_id, msg.id(), text).reply_markup(kb).await?; // Используем поле

               {
                   ...
                   if let Some(current_state @ UserState::AwaitingHedgeAssetSelection { .. }) = state_guard.get_mut(&chat_id) {
                        *current_state = UserState::AwaitingHedgeSum {
                           ...
                           last_bot_message_id: Some(msg.id().0), // Используем поле
                       };
                       info!("User state for {} set to AwaitingHedgeSum for {}", chat_id, symbol);
                        ...
                        let _ = navigation::show_main_menu(&bot, chat_id, Some(msg.id())).await; // Используем поле
                   }
               }
           } else {
...
where
    E: Exchange + Clone + Send + Sync + 'static,
{
   let chat_id = msg.chat.id; // Поле
   let message_id = msg.id;   // Поле
       ...
       if let Some(payload) = data.strip_prefix(callback_data::PREFIX_HEDGE_CONFIRM) {
            if payload == "yes" {
               ...
               let (symbol, sum, volatility_fraction) = {
                   let state_guard = state_storage.read().expect("Lock failed");
                    match state_guard.get(&chat_id) {
                        ...
                        _ => {
                            ...
                            bot.answer_callback_query(q.id).text("Состояние изменилось, начните заново.").show_alert(true).await?;
                            let _ = navigation::show_main_menu(&bot, chat_id, Some(msg.id())).await; // Поле
                             { state_storage.write().expect("Lock failed").insert(chat_id, UserState::None); }
                            return Ok(());
                        }
                    }
                ...
                bot.edit_message_text(chat_id, msg.id(), waiting_text) // Поле
                ...
                let hedger = Hedger::new((*exchange).clone(), (*cfg).clone());
                 ...
                 match hedger.calculate_hedge_params(&hedge_request).await {
                    Ok(params) => {
                        ...
                        // ИСПРАВЛЕНО: Передаем msg (MaybeInaccessibleMessage) вместо msg.clone() (Message)
                        spawn_hedge_task(
                            bot.clone(), exchange.clone(), cfg.clone(), db.clone(),
                            running_operations.clone(), chat_id, params, sum,
                            volatility_fraction * 100.0, msg, // Передаем msg напрямую
                        ).await;
                    }
                    Err(e) => {
                        ...
                        let _ = bot.edit_message_text(chat_id, msg.id(), error_text) // Поле
                                    .reply_markup(navigation::make_main_menu_keyboard())
                                    .await;
                    }
                }
